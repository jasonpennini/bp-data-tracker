...
const MaxEVBlackbox = () => {
  ...
  useEffect(() => {
    const fetchBPWorkouts = async () => {
      ...
      const filteredEntries = json.filter(
        (entry) => entry.bpType === "Blackbox",
      );
      // Sort bpEntries by highest exit speed first
      const sortedEntries = filteredEntries.sort(
        (a, b) => b.exitSpeed - a.exitSpeed,
      );
