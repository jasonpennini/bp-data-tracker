...
const BPTable = ({bpEntries})  => {
    ...
    // Define a function to conditionally format the cell based on the task due date
    const getCellClassName = (params) => {
      const currentDate = new Date();
      const dueDate = new Date(params.row.dueDate);
      // if the current date is byond the due date, cellClassName will be past-due-cell, otherwise it will be an empty string
      return currentDate > dueDate ? 'past-due-cell' : '';
    };
        ...
        return (
          <div style={{ height: 600, width: '100%' }}>
            <DataGrid
              ...
              checkboxSelection
              // adding a className for each row in our table
